...
void matrixtosend(int rank, int **values, double **A, double **B, int a, int b, int q){

	int pack_size;
	char *pack_buff;
	int pack_position;

	double **matrixa = new double *[a];
	for (int i = 0; i < a; i++)
...
int main(int argc, char *argv[]) {
	...
	if (rank == 0) {
		...
		for (int i = 1; i < size; i++){
			matrixtosend(i, Values[i], A, B, a, b, q);
		}
